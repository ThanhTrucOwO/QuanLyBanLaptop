...
namespace DAO
{
    public class QuyenDAO
    {
        ...
        public QuyenDTO quyen(int idQuyen)
        {
            conn.Conn.Open();
            string query = "select * from Tbl_Quyen where idQuyen = @idQuyen";
            QuyenDTO q = null;
            SqlCommand command = new SqlCommand();
            command.Connection = conn.Conn;
            command.CommandText = query;
            command.Parameters.AddWithValue("@idQuyen", idQuyen);
            SqlDataReader reader = command.ExecuteReader();
            if (reader.HasRows)
            {
                reader.Read();
                string nameAuth = reader["nameAuth"].ToString();
                q = new QuyenDTO(idQuyen, nameAuth);
            }
            conn.Conn.Close();
            reader.Close();
            return q;
