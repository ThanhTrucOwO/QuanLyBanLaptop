...
namespace GUI
{
    public partial class FormEmployee : Form
    {
        ...
        public void initializationDataServiceEmployee()
        {
            string[] service;
            if (auth == "admin")
                 service = new string[] { "Quản lý","Nhân viên"};
            else
                service = new string[] {"Nhân viên" };
        ...
        public void setDataEmployee(List<NhanVien> list)
        {
            ...
            {
                if (this.auth == "manager" && item.ServiceEmployee == "Quản lý"
                    || item.ServiceEmployee == "admin")
                    continue;
                string[] row = new string[] { item.Id, item.Name,item.ServiceEmployee,item.Birthday.ToString(),item.Phone,
                    changeMoney(item.SalaryEmployee.ToString("0"))
                   };
        ...
        public void setDataTopEmployee()
        {
            ...
            foreach (NhanVien item in list)
            {
                ...
                if (cout == 10)
                    break;
                string[] row = new string[] {item.Name,item.QuantityBillPay.ToString(),changeMoney(item.TotalSales.ToString("0")) };
        ...
        public void displayInfoEmployee(string id, string name, string service, string birthday, string phone, string salary)
        {
            ...
            inp_salary.Text = changeMoney(float.Parse(salary).ToString("0"));
        ...
        public void eventClickEmployee(string idEmployee, int index)
        {
            if (index == this.index)
            {
                ...
                pnlBody.Controls.Add(fInfoEmployee);
