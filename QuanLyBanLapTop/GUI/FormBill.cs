...
namespace GUI
{
    public partial class FormBill : Form
    {
        ...
        private void btn_export_Click(object sender, EventArgs e)
        {
            ...
            if (tb_bill.RowCount > 1)
                if (tb_detailsBill.RowCount > 1)
                    if (tb_bill.CurrentRow != null)
                    {
                        string result = exportBill(this.idBill);
                        if(result == "success")
                        MessageBox.Show("Đã in hoá đơn, mã hoá đơn" + idBill, "Thông báo", MessageBoxButtons.OK);
                        else
                        {
                            MessageBox.Show("Hoá đơn đã được in vui lòng kiểm tra lại", "Thông báo", MessageBoxButtons.OK);
                        }
                    }
                    else
                        MessageBox.Show("Chưa chọn hoá đơn", "Thông báo", MessageBoxButtons.OK);
        ...
        private void btn_pay_Click(object sender, EventArgs e)
        {
            if (this.idBill != -1)
            {
                ...
                if (b.Status != "Đã thanh toán")
                {
                    ...
                    pnlBody.Controls.Add(fInfoPayment);
        ...
        private void txt_detailsBill_Click(object sender, EventArgs e)
        {
