...
namespace BUS
{
    public class ImportProduct
    {
        ...
        public static string exportReceiptExcel(int idReceipt)
        {
            List<ChiTietPhieuYeuCauNhapHangDTO> listDetailsReceipt = listDtCoupon(idReceipt.ToString());
            List<SanPham> listP = new List<SanPham>();
            foreach (ChiTietPhieuYeuCauNhapHangDTO value in listDetailsReceipt)
            {
                listP.Add(new SanPhamDAO().searchId(value.IdProduct));
            }
            PhieuYeuCauNhapHang p = new PhieuYeuCauNhapHangDAO().coupon(idReceipt);
            return exportExcelReceipt(p, listDetailsReceipt, listP);
        }
